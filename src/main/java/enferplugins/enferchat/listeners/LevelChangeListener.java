...
public class LevelChangeListener implements Listener {
    ...
    public LevelChangeListener(EnferChatPlugin plugin) { this.plugin = plugin; }
    ...
    private void handleLevelUp(Player player, PlayerLevelChangeEvent event) {
        if (player != null) sendIfNotEmpty(player, plugin.getConfig().getString("formats.level-up.player", ""),
            s -> s.replace("{level}", String.valueOf(event.getNewLevel())).replace("{old_level}", String.valueOf(event.getOldLevel())));
        String broadcast = plugin.getConfig().getString("formats.level-up.broadcast", "");
        if (!broadcast.isBlank()) Bukkit.broadcastMessage(ColorUtil.colorize(broadcast
            .replace("{name}", event.getPlayerName()).replace("{level}", String.valueOf(event.getNewLevel())).replace("{old_level}", String.valueOf(event.getOldLevel()))));
    }

    private void handleDeathReset(Player player, PlayerLevelChangeEvent event) {
        if (player == null) return;
        sendIfNotEmpty(player, plugin.getConfig().getString("formats.death-reset.player", ""),
    ...
    private void handleAdminChange(Player player, PlayerLevelChangeEvent event, String key) {
        ...
        sendIfNotEmpty(player, plugin.getConfig().getString("formats." + key + ".player", ""),
            s -> s.replace("{amount}", String.valueOf(amount)).replace("{old_level}", String.valueOf(event.getOldLevel())).replace("{level}", String.valueOf(event.getNewLevel())));
    }

    private void handleAdminSet(Player player, PlayerLevelChangeEvent event) {
        if (player == null) return;
        sendIfNotEmpty(player, plugin.getConfig().getString("formats.admin-set.player", ""),
            s -> s.replace("{old_level}", String.valueOf(event.getOldLevel())).replace("{level}", String.valueOf(event.getNewLevel())));
    }

    private void handleAdminReset(Player player, PlayerLevelChangeEvent event) {
        if (player == null) return;
        sendIfNotEmpty(player, plugin.getConfig().getString("formats.admin-reset.player", ""),
            s -> s.replace("{old_level}", String.valueOf(event.getOldLevel())));
    }
